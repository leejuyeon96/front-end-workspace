...
-class : 현재 문서내에 "중복"된 값 작성 가능, "여러 개" 작성 가능
...
1. **Block**
    ...
    - 특정한 콘텍스트에 의존하지 않으며 어디에서도 재사용 할 수 있는 부품
    - 느슨하게 결합
...
<ul class="menu">
    ...
    <li class="menu__item--selected">항목2</li>
...
- none : 변환 없이 표시
- capitalize : 시작하는 것 첫 번째 글자를 대문자로 변환
...
- pre-wrap : 여러 개의 공백을 그대로 표시, 영억을 넘어가는 내용은 자동으로 줄 바꿔 표시
...
- 수평위치 : lest, center, right, 크기값(px), 백분율로 표시
...
-padding-box : 테두리를 뺀 패딩이 기준(기본값)
-content-box : 내용content이 기준
...
## 박스 모델(Box Model)
- 모든 요소들은 사각형 박스로 표현되는데 스타일 시트에서는 박스 형태인 요소를 박스 모델 이라고 한다.
- css를 사용하면 박스 모델 요소의 크기, 위치, 속성을 변경할 수 있다.
- 박스 모델은 콘텐츠(content) 영역과 테두리와 콘텐츠 영역 사이의 여백인 패딩, 박스 모델 사이의 여백인 마진으로 구성된다.
...
- inline 요소에는 margin, padding, height, width가 제대로 설정되지 않는다.
...
- collapse : 표의 행, 열, 행 그룹 등에서 지정하면 서로 겹치도록 지정한다. 그 외의 영역에서는 사용하면 hidden처럼 처리된다.
