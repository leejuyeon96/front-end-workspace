...
$('h4').eq(4).text('테스트-5');
// 클래스 선택자 : test를 선택해서 홀수 인덱스 요소만 배경색상: royalblue, 글자색: white
$('.test').odd().css({'background-color': 'royalblue', 'color': 'white'});
//클래스 선택자 : test를 선택해서 짝수 인덱스 배경색상: hotpink, 글자색: white
$('.test').even().css({'background-color': 'hotpink', 'color': 'white'});
//h4 중 클래스 선택자 test 아닌 요소 선택해서 배경색상: chocolate, 글자색: white
...
//span 요소의 부모 요소에 스타일 글자색: red border: 2px solid
$('span').parent().css({'color': 'red', 'border': '2px solid'});
//li 요소의 모든 상위 요소에 스타일 적용 글자색: blue
$('li').parents().css({'color': 'blue'});
//li 요소의 상위 요소 중 div만 스타일 적용 border: 굵기 2px 스타일 dashed 색상 tomato
$('li').parents('div').css({'border': '2px dashed tomato'});
//span 요소부터 div 요소이전까지 상위 요소 스타일 적용 배경색: lavenderblush
$('span').parent('div').css({'background-color': 'lavenderblush'});
...
//.wrap인 요소의 자식 요소들에 style1 
$('.wrap').children().css(style1);
//.wrap 자식의 자식 요소들에 style2
$('.wrap').children().children().css(style2);
//.wrap 자식의 자식 요소중 ul 태그의 style3
$('.wrap').children().children('ul').css(style3);
//.wrap 자식의 자식의 자식 요소 중 li 태그의 style4
$('.wrap').children().children().children('li').css(style4);
//.wrap 모든 하위 요소 중 span 태그의 style 5
...
// h2 요소의 이전형제 요소에 style5
$('.wrap2 h2').prev().css(style5);
//h2 요소 이전에 오는 모든 형제 요소에 배경색: moccasin
$('.wrap2 h2').prevAll().css('background-color', 'moccasin');
//h2 요소 이전부터 p 요소 이후까지 모든 형제 요소에 글자 크기 2rem
